...
import ProductDetailsDialogue from "@/components/shoping-view-c/product-details";
  ...
  const ShopingListing = () => {
    ...
    console.log("The Product", productList);
     console.log("The filter", filters, "The param will be", SearchParams, "ths sort also", sort);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
            {productList && productList.length > 0
              ? productList.map((productItems) => (
                  <ShoppingProductTile handleGetProductDetails={handleGetProductDetails} product={productItems} />
                ))
              : null} 
          </div>
        </div>
        <ProductDetailsDialogue open={openDetailsDialog} setOpen={setopenDetailsDialog} productDetails={productDetails} />
      </div>
    );
  };
